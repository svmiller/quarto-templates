...

{
   ...
   $if(acmsans)$
$if(anonymous)$\hfill $else$
$for(by-author)$\MakeUppercase{\textsf{\large $by-author.name.literal$}}, 
\small{
...
$else$
$if(anonymous)$\hfill $else$
$for(by-author)$\MakeUppercase{$by-author.name.literal$}, 
\small{
$for(by-author.affiliations)$
$if(by-author.affiliations.name)$
$by-author.affiliations.name$
$endif$
$endfor$
}  $sep$ \par \vskip -3.5pt $endfor$ $endif$
$endif$
